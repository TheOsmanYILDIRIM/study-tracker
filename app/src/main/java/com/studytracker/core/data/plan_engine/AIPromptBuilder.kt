...
object AIPromptBuilder {
    ...
    fun buildPrompt(
        ...
        return buildString {
            ...
            appendLine("1. Video linklerini doğrudan satır sonuna ekle (| https://youtu.be/...).")
            appendLine("2. ANKİ GÖREVLERİ: Anki'nin kendi aralıklı tekrar (SRS) algoritması ve günlük kart takibi zaten vardır. 'Şu kadar kart çöz', 'haftalık tekrar' gibi hedefler yazma! Yalnızca 'Anki Temizle: <Deste Adı>' formatında yaz (Örn: 'Anki Temizle: İngilizce - 1. Ay | 15 dk').")
            appendLine("3. Günlük çalışma sürelerini öğrenciyi sıkmayacak şekilde dengeli ve sade tut.")
            appendLine()
            ...
            appendLine()
            appendLine("[DERSLER]")
            appendLine("mat_video = Matematik - Üslü Sayılar Video | 35 dk | https://youtu.be/example")
            appendLine("anki_ing = Anki Temizle: İngilizce - 1. Ay | 15 dk")
            appendLine("fizik_video = Fizik - Fizik Bilimine Giriş | 30 dk | https://youtu.be/example")
            appendLine("anki_cog = Anki Temizle: Coğrafya - 1. Ay | 15 dk")
            appendLine("kitap = Serbest Kitap Okuma | 25 dk")
            appendLine()
            appendLine("[GUNLER]")
            appendLine("Pazartesi = mat_video, anki_ing, kitap")
            appendLine("Sali = fizik_video, anki_cog, kitap")
            appendLine("Carsamba = mat_video, anki_ing, kitap")
            appendLine("Persembe = fizik_video, anki_cog, kitap")
            appendLine("Cuma = mat_video, anki_ing, kitap")
            appendLine("Cumartesi = fizik_video, anki_cog, kitap")
            appendLine("Pazar = kitap")
            appendLine()
            appendLine("[HAFTALIK]")
            appendLine("deneme = Hafta Sonu Mini Pekiştirme | 30 dk | Genel Tekrar")
            appendLine()
            appendLine("---")
