...
### [2026-09-15] Tamamlandı: Cyber-Violet Dark Futuristic Tasarım Sistemi ve Kapsamlı Ticker İzolasyonu
- **Cyber-Violet Koyu Mor Tasarım Dili:** Gözü yoran beyaz/açık lavanta zemin tamamen terk edildi. Derin Cyber-Violet arka plan (`#090414`), Koyu Cam Kartlar (`#140C28`), 32dp derin yuvarlatılmış Hero gradyanı (`#6D28D9` ➔ `#9333EA` ➔ `#D946EF`), yüksek kontrastlı Neon Mint (`#00F5D4`) pill butonlar, 18dp squircle yarı saydam neon ikon kutuları ve `%20` saydam mor cam kenarlıklar (`ZomoDarkBorder`) uygulandı.
- **Recomposition Ticker İzolasyonu:** `SessionStateManager` içindeki saniyelik sayaç (`elapsedSeconds`) ayrı bir `MutableStateFlow<Long>` akışına taşındı. `_activeState` (başlık, mola durumu, ekran görüntüsü sayısı) artık saniyede 1 güncellenmeyip yalnızca durum değişikliklerinde tetikleniyor. Böylece tüm ana ekranların ve kartların her saniye baştan hesaplanması tamamen durduruldu.
- **LazyColumn Donanım Yükü Sıfırlama:** Liste kartlarındaki ağır ve GPU'yu kilitleyen `Modifier.shadow(..., spotColor = ...)` çağrıları kaldırıldı; 120 FPS akıcı kaydırma için donanım hızlandırmalı düz cam kenarlıklar (`BorderStroke`) uygulandı.
- **DerivedStateOf Fazlalıklarının Temizliği:** `ChildHomeScreen` ve `ParentDashboardScreen` üzerindeki filtreleme listeleri doğrudan `remember(occurrences)` içine alınarak bellek tahsisatı ve nesne çöpü sıfırlandı.
